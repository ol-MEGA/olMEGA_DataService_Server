...
class FeatureService():
    ...
    def run(self):
        ...
        Features = self.db.execute_query(query)
        query = "SELECT * FROM EMA_filetype"
                            ...
                            if item["datachunkid"] != lastItem["datachunkid"] and len(files) > 0:
                                ...
                                for value in values:
                                    if type(value) is dict and "value" in value and "start" in value and "end" in value and "isvalid" in value:
                                        ...
                                        header = bytearray()
                                        header += value["Frames"].to_bytes(4, "big")
                                        header += value["Dim"].to_bytes(4, "big")
                                        ...
                                        if len(header) != 36:
                                            ...
                                            content = bytearray()
                                            for val in numpy.concatenate((numpy.zeros([value["Frames"], 2]),  value["value"]), axis = 1).flatten():
